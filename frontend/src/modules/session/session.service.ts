...
import { queryPageSession } from '@/modules/session/graphql/session.graphql';
...
class ServiceSessionClass implements ServiceCollectionInterface<Session> {
    useLastSession({
        analogOnly = false,
        digitalOnly = false,
    }: {
    ...
    async loadPage(data: InputCollection) {
        return loadPageBase<
            Session,
            { sessions: ServiceCollectionLoadPage<Session> }
        >({
            data,
            query: queryPageSession,
            parseResult: async (response) => ({
                items: await Promise.all(
                    response.sessions.items.map((session: Session) =>
