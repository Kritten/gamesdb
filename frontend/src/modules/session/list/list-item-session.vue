...
export default defineComponent({
  ...
  setup(props) {
    const starts = props.session.playtimes.map((playtime) => playtime.start);
    const ends = props.session.playtimes.map((playtime) => playtime.end).filter((value) => value !== null) as Array<Date>;

    const start = starts.sort((playtime1, playtime2) => compareAsc(playtime1, playtime2))[0];
    const end = ends.sort((playtime1, playtime2) => compareDesc(playtime1, playtime2))[0];
