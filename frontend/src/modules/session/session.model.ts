...
export class Session extends Entity implements SessionInterface {
    ...
    constructor(data: SessionInterface = {}) {
        super(data);
        this.comment = setDefaultIfNullOrUndefined<string | null>(
            data.comment,
            null
        );
        this.isChallenge = setDefaultIfNullOrUndefined<boolean>(
            data.isChallenge,
            false
        );
        this.isVirtual = setDefaultIfNullOrUndefined<boolean>(
            data.isVirtual,
            false
        );
        this.game = data.game;
        ...
        this.playtimes = setDefaultIfNullOrUndefined<Playtime[]>(
            data.playtimes,
            []
        );
    }
    ...
    stop() {
        const indexCurrentPlaytime = this.playtimes.findIndex(
            (playtime) => playtime.end === null
        );
    ...
    static async parseFromServer(data: EntityInterface): Promise<Session> {
        ...
        if (entity.players !== undefined) {
            entity.players = entity.players.map((player) =>
                playerById(player.id as ID)
            );
        }

        if (entity.winners !== undefined) {
            entity.winners = entity.winners.map((winner) =>
                playerById(winner.id as ID)
            );
        }
