...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const removeSortBy = (index: number) => {
            emit(
                'update:sortBy',
                props.sortBy.filter((item, indexItem) => indexItem !== index)
            );
            emit(
                'update:sortDesc',
                props.sortDesc.filter((item, indexItem) => indexItem !== index)
            );
        };
        ...
        return {
            ...
            sortDescSingle,
            // @ts-ignore
            optionsSortByPrepared: computed(() =>
                props.optionsSortBy.map((value) => ({
                    value: value.field,
                    label: value.name,
                }))
            ),
