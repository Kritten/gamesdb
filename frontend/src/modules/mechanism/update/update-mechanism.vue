<template>
  <form @submit.prevent="updateMechanism.update">
    <div>
      <label for="name">{{ t('mechanism.label') }}</label>
      <input
        id="name"
        v-model="updateMechanism.mechanism.value.name"
      >
    </div>
    <div>
      <button type="submit">
...
export default {
  ...
  setup(context) {
    const { t } = useI18n();
    const updateMechanism = ServiceMechanism.useUpdate(context.mechanism);
